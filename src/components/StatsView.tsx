...
import { computeStreaks, countInMonth, daysSinceFirst, groupByDay, lastNDaysCounts } from '../lib/stats'
...
export function StatsView({ checkins }: Props) {
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-2 rounded-lg bg-neutral-100 p-1 text-sm dark:bg-neutral-900">
        {(
          [
            ['overview', '概览'],
            ['analysis', '分析'],
      ...
      {subTab === 'overview' ? (
        <div className="space-y-6">
          {firstDate && (
            <div className="rounded-xl bg-violet-50 p-4 text-center dark:bg-violet-950">
              <p className="text-2xl font-semibold text-violet-800 dark:text-violet-300">{totalDays} 天</p>
              <p className="mt-1 text-xs text-violet-600 dark:text-violet-400">
                自 {format(firstDate, 'yyyy-MM-dd')} 使用至今
              </p>
            </div>
          )}
