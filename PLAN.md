...
- **创建日期**：2026-09-22
- **最后更新**：2026-09-26
- **当前阶段**：完成一轮全面测试，修复 14 个 bug（其中 7 个在同步层，会导致丢数据/漏同步）；等待用户执行 Supabase 建表 SQL 并清理实测时产生的重复分类
...
| P1 | 更深入的数据分析（不只是基础统计） | ✅ 已完成（线性回归、同比环比、活跃度热力图等） |
...
- **同步策略**：本地优先 + 定期/触发式双向同步。单用户场景，冲突处理用简单的"最后推送者获胜"（`updated_at` 在推送时盖戳），没有做复杂的 CRDT
...
src/
  ...
  lib/
    ...
    analysis.ts          # 深入分析（星期/月度/时段分布、线性回归、同比环比、活跃度热力图数据）
  ...
  hooks/
    ...
    useCurrentDay.ts  # 当前本地日期，过午夜/切回前台时自动刷新
  ...
  components/
    ...
    AnalysisView.tsx      # 分析子栏目：规律性、趋势、同比环比、活跃度热力图、分布图表
...
2. `syncNow(userId)`（顺序：分类 push → 分类 pull → 打卡 push → 打卡 pull）：
   - **push**：把所有 `dirty` 的本地记录分批 `upsert`（每批 1000 条），并把 `updated_at` 设为**推送时刻**（不是打卡时刻），这样离线很久才推上去的记录，其他设备也能按"上次同步之后有更新"拉到。只清除推送期间没再被改动过的行的 `dirty` 标记
   - **pull**：分页拉取（每页 1000 条，Supabase 单次上限）`updated_at` 晚于"上次同步时间 − 5 分钟"的记录（多拉 5 分钟容忍设备时钟误差，重复写入是幂等的）。本地有未推送改动时，只有服务器版本**严格更新**才覆盖（按时间值比较，不比字符串）
   - 拉到 `category_id` 为空的记录（旧版本客户端写的）会自动归到历史数据分类，并回推修正
3. 触发时机：登录后立即同步一次；每 30 秒轮询一次；浏览器 `online` 事件触发一次；任意本地写入后触发一次（防抖）。同一时间只跑一次同步，期间的新请求会排到结束后
4. 删除是软删除（`deleted: true` 字段），保证删除动作也能正常同步到其它设备，不会被同步逻辑复活
5. 退出登录会清空本机数据；有未同步记录时会先弹框确认

### 2.3 历史数据导入是怎么工作的

- 数据源：旧的打卡记录表格（Excel），只有每天的打卡**次数**，没有具体时间
- 导入时给每天的记录在 8:00–22:00 之间均匀生成时间点，备注统一写 `历史导入`
- 已有真实记录的日期（按**本地日期**判断）会自动跳过，不会重复计数
- 只需在"统计 → 概览"里点一次"导入历史数据"。"是否已导入"根据数据判断：该分类里只要存在"历史导入"备注的记录（包括其他设备导入后同步下来的）就不再显示；至少完成一次同步拉取之前也不显示，避免新设备在数据还没拉下来时重复导入
...
### Phase 7：今日页提醒 + 分析页调整 ✅ 已完成（2026-09-25）

- [x] 今日页新增"你已经 X 天没打卡了"提示，固定显示在底部导航栏上方，不随打卡按钮居中；今天打过卡就显示"你已经 0 天没打卡了"（不换成别的文字），从没打过卡才显示"还没有打卡记录"
- [x] 修了 `daysSinceLastCheckIn` 遇到未来日期记录时会算出负数的 bug，加了 `Math.max(0, …)` 兜底
- [x] 分析页去掉 K-Means 活跃度聚类和季节性两块（用户反馈"感觉没啥意义"），换成"同比/环比"（本周比上周、本月比上月，带涨跌箭头和百分比）和"活跃度热力图"（GitHub 贡献图风格，周历网格 + 颜色深浅）
- [x] 删除了不再使用的 `charts/ScatterChart.tsx`，新增 `charts/HeatmapChart.tsx`
- [x] 修了热力图的一个真实 bug：SVG `<rect>` 用 `background` 不生效（得用 `fill`），一开始格子全渲染成黑色

### Phase 8：全面测试与 bug 修复 ✅ 已完成（2026-09-26）

用户要求"进行一个全面测试找 bug"。方法：逐文件代码审查 + 浏览器内集成测试（在页面里直接 import 真实的 `sync.ts`，把 Supabase client 换成一个模拟服务器，用真实 IndexedDB 跑），共 33 项检查全部通过。

**同步层（最严重，会丢数据/漏数据）**：
- [x] **超过 1000 条只能同步前 1000 条**：Supabase 单次查询默认最多返回 1000 行，而这个账号已有 962+ 条，马上就会超。新设备首次同步会只拿到前 1000 条，之后 `lastSyncedAt` 一更新，剩下的就永远拉不下来了。→ 改成分页拉取（每页 1000 条，按 `updated_at, id` 排序）；推送也分批
- [x] **离线打的卡，另一台设备收不到**：`updated_at` 原来记的是打卡时间，另一台设备按"`updated_at` > 我上次同步时间"来拉。手机离线打卡、隔天才联网推上去，这条的 `updated_at` 早于电脑的上次同步时间 → 电脑永远拉不到。这正是"没网也能打卡"这个核心场景。→ 推送时把 `updated_at` 改成推送时刻（打卡时间 `checked_at` 不变），并且拉取时往前多拉 5 分钟容忍设备间时钟误差
- [x] **同步进行中的修改会丢**：推送请求在路上时用户又改了同一条，推送完成后会把 dirty 标记整个清掉，这次新改动就被当成"已同步"再也不会推。→ 只清除推送后没再被改过的行
- [x] **时间戳按字符串比较不可靠**：Supabase 返回 `+00:00` 后缀，客户端写的是 `Z`，冲突判断直接比字符串会判错；同毫秒时还会让服务器旧版本覆盖本地新改动。→ 按时间值比较，平局时本地未推送的改动优先
- [x] **同步可能重叠执行**：30 秒轮询和打卡后触发的同步可能同时跑，两次推送/拉取交错。→ 加了进行中锁，期间新的请求排队到结束后再跑一次
- [x] **旧版本客户端写入的无分类记录会"消失"**：PWA 有缓存，旧版本页面（没有分类功能）写入的记录 `category_id` 为空，新版本所有页面都按分类过滤，这些记录就看不到了。→ 拉取时空分类自动归到历史数据所属分类并回推修正；启动时也会把本地所有无分类记录归位
- [x] **退出登录会静默丢弃未同步数据**：退出时会清空本地库，如果有离线打的卡还没推上去就没了。→ 有未同步记录时弹确认框提示条数

**历史数据导入**：
- [x] **跳过已有日期的判断用的是 UTC 日期**：`checkedAt.slice(0,10)` 取的是 UTC 日期，日本时间早上 9 点前的打卡会被算成前一天，导致该跳过的那天没跳过、另一天被误跳过。→ 改用本地日期（`dayKey`）。测试：9/22 早 8 点的记录，修前会误跳过 9/21（剩 961 条），修后正确跳过 9/22（剩 959 条）
- [x] **第二台设备可能重复导入**："是否已导入"原来只记在本机，换设备或重新登录后导入卡片会再次出现，点了就会多出一整套 962 条重复数据。→ 改为根据数据判断（该分类里只要有"历史导入"备注的记录就算已导入，会随同步到每台设备），并且在至少完成一次同步拉取之前不显示导入卡片；导入函数本身执行时也会再检查一遍
- [x] 导入卡片改成响应式（`useLiveQuery`），其他设备的导入同步下来后卡片会自动消失

**日期 / 界面**：
- [x] **可以给未来日期打卡**：日历能翻到未来月份并补录，也能在今天补录一个比现在晚的时间，这正是之前"-7 天没打卡"那个 bug 的根源，也会让分析图表错乱。→ 未来日期显示"未来的日期还不能打卡"且不给补录入口；补录/编辑时间晚于当前时刻时禁用按钮并提示
- [x] **过了午夜"今日"不更新**：PWA 在后台放一晚再打开，今日页还显示昨天的日期，此时打卡虽然记录的是正确时间，但不会出现在"今日"列表里。→ 新增 `useCurrentDay` hook，切回前台/获得焦点/每分钟检查一次日期
- [x] **编辑表单显示旧值**：打卡记录被同步更新后，点开编辑框里还是这一行刚挂载时的旧时间/备注。→ 打开编辑时从最新记录重新取值
- [x] **中文输入法按回车会提前提交**：选择事件页的新建/改名输入框，用输入法打中文时按回车选字就会直接提交半截名字。→ 输入法组字中的回车不触发提交
- [x] 迁移单例按用户区分（同一标签页里换账号登录时也会重新跑迁移）
...
- 冲突解决是简单的"最后推送者获胜"，没有处理同一条记录在两台离线设备上被分别修改后的合并（极端场景，个人使用概率很低）
- 同步依赖各设备时钟大致准确（拉取有 5 分钟容差）；手机/电脑都开着网络自动校时的话不是问题
- 同步层的修复用浏览器内模拟服务器测过，但还没在真实 Supabase 上跑过——需要你执行 Phase 6 的建表 SQL 后实际用一下确认
...
| 2026-09-25 | "没打卡天数"提示的显示方式 | ✅ 固定显示具体数字（含 0），不因今天打卡与否切换文案 | 用户明确要求"无论今天打没打过都显示"、"今天打卡了就显示0"——中途我自作主张换成了"你今天已经打卡了"这种文字，被纠正回数字 |
| 2026-09-25 | 分析页替换 K-Means 聚类 / 季节性 | ✅ 换成同比环比 + 活跃度热力图 | 用户反馈这两块"感觉没啥意义"；同比环比给出可直接行动的"最近变勤快/变懒"信号，热力图比月度平均值更能一眼看出全年的疏密和断档，都不需要用户懂统计概念就能看懂 |
| 2026-09-26 | 离线记录同步修复方式 | ✅ 客户端推送时给 `updated_at` 盖推送时刻的戳（不改 `checked_at`），不用服务器触发器 | 触发器方案更"正确"但需要用户再跑一次 SQL；客户端方案自包含、单用户场景下效果等价 |
| 2026-09-26 | 未来日期打卡 | ✅ 禁止（未来日期不给补录入口，补录/编辑时间不能晚于当前） | 打卡是记录已发生的事；允许未来日期正是之前"-7 天没打卡"的根源，也会让时序分析错乱 |
| 2026-09-26 | "是否已导入历史数据"的判断依据 | ✅ 看数据里有没有"历史导入"记录，不再用本机标记 | 本机标记换设备/重新登录就没了，会导致重复导入一整套 962 条 |
...
2. **历史导入时间估算**：现在是 8:00–22:00 均匀分布，如果发现和实际习惯差异较大，要不要改成别的估算策略（比如更贴近星期分布反映出的模式）？
...
| 2026-09-25 | 分析页移除 K-Means 活跃度聚类和季节性两块（用户反馈"感觉没啥意义"），换成：①"同比/环比"卡片——本周 vs 上周、本月 vs 上月的次数对比，带涨跌箭头和百分比，比聚类档位更直接能看出"最近是变勤快了还是变懒了"；②"活跃度热力图"——GitHub 贡献图风格的周历网格，颜色深浅代表当天次数，一眼看出全部历史的疏密和断档，比"季节性"按月平均更直观。新增 `charts/HeatmapChart.tsx`，删除了 `charts/ScatterChart.tsx`（连带用不到了）。开发时还真踩了一个坑：SVG `<rect>` 只认 `fill` 不认 `background`，一开始热力图格子全渲染成黑色，测试时发现并修了 |
| 2026-09-26 | 全面测试，修复 14 个 bug（详见 Phase 8）。同步层 7 个：超过 1000 条只同步前 1000 条、离线打卡另一台设备收不到、推送途中的修改丢失、时间戳字符串比较不可靠、同步重叠执行、旧客户端无分类记录消失、退出登录静默丢未同步数据。其他：历史导入按 UTC 日期判断跳过日、换设备可能重复导入、可给未来日期打卡、过午夜今日页不更新、编辑框显示旧值、中文输入法回车提前提交、迁移单例未按用户区分。33 项浏览器内检查全部通过 |
