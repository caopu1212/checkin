...
| 2026-09-25 | 今日页新增"你已经 X 天没打卡了"提示，固定显示在页面最下方（导航栏上方），不随打卡按钮一起纵向居中。中间改过两版：一开始当天已打卡就隐藏（用户反馈没看到），后来改成隐藏时换成"你今天已经打卡了"的文字（用户纠正：要看到具体数字，今天打过就该显示"0"，不要换成别的文字），现在固定显示 `你已经 {N} 天没打卡了`，今天打过卡 N 就是 0，只有从没打过卡时才换成"还没有打卡记录"。顺带修了一个真实 bug：`daysSinceLastCheckIn` 遇到未来日期的记录（本地脏数据触发）会算出负数，加了 `Math.max(0, …)` 兜底 |
