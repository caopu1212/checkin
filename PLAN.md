...
| 2026-09-25 | ~~今日页新增"你已经 X 天没打卡了"提示~~：当天已打卡时不显示，用户反馈没看到；改成不管今天打没打卡都常驻显示在页面最下方（今天打过显示"你今天已经打卡了"，从没打过显示"还没有打卡记录"），位置固定在底部导航栏上方，不随按钮一起纵向居中；同时修了一个真实 bug：`daysSinceLastCheckIn` 遇到未来日期的记录（本地脏数据触发）会算出负数，加了 `Math.max(0, …)` 兜底 |
