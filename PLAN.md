...
5. ~~多设备并发建分类的边界情况~~：已修复——迁移前会先查一次云端有没有别的设备已经建过分类，有就直接采用，不会各建一套。仍然建议第一次升级后**先在一台设备上打开、等同步状态显示"已同步"，再打开另一台**，避免两台设备都离线/都没同步过时仍可能各自创建。
...
| 2026-09-23 | 选择事件页的事件名称居中显示，方便单手点击 |
